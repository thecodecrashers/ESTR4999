...
# 训练结束后，保存共享的嵌入层
torch.save(embedding_net.state_dict(), os.path.join("meta_sac", "shared_pretrained_embedding.pth"))
...
for meta_iter in range(num_meta_iterations):
    ...
    def reptile_update(meta_model, task_models, lr):
        ...
        with torch.no_grad():
            for key in meta_dict.keys():  # 遍历 meta_model 的参数名
                task_params = torch.stack([task_dict[key] for task_dict in task_dicts])  # 取出所有任务的参数
                avg_param = torch.mean(task_params, dim=0)  # 计算均值
                meta_dict[key] += lr * (avg_param - meta_dict[key])  # 进行 Reptile 更新

        meta_model.load_state_dict(meta_dict)  # 加载更新后的参数
