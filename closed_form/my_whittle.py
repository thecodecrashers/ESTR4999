...
def whittle_index(a_max, d_max, lambd, tol=1e-6, max_iter=100000, w_tol=1e-4, w_iter=50):
    ...
    with tqdm(total=a_max * d_max, desc="Computing Whittle Index", unit="points") as pbar:
        for a in range(1, (a_max + 1)//50):
            for d in range((d_max + 1)//50):
                low, high = -10, 10  # 设定初始搜索范围
                while high - low > w_tol:
                    ...
                    mid=mid = d + (1 - lambd) * f_mid[min(a + 1, a_max), d] + lambd * f_mid[1, min(d + a, d_max)] - (1 - lambd) * f_mid[min(a + 1, a_max), 0] - lambd * f_mid[1, min(a, d_max)]
    ...
    with open("my_whittle_indices.json", "w") as f:
        json.dump(whittle_indices_dict, f)
    
    return whittle_indices

# 示例调用
whittle_indices = whittle_index(500, 500, 0.5)
